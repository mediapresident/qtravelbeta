...
class Curl extends CApplicationComponent{
	...
	*/
	public function run($url,$GET = TRUE,$POSTSTRING = array(), $unsafe = FALSE){
                $this->setUrl($url);
                if( !$this->url )
		...
		self::checkOptions($this->options,$this->validOptions);
                ...
                if(!$unsafe)$this->setOption(CURLOPT_POSTFIELDS, $this->cleanPost($POSTSTRING));
				else $this->setOption(CURLOPT_POSTFIELDS, $POSTSTRING);        
                }
