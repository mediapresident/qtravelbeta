<div class="row" style="position:relative; top: -15px;">
    ...
    <div class="col triple">
        ...
        <?php if ($model->key_points): ?>
            <h3>QT Insider</h3>
            <?php echo $model->key_points; ?>
        <?php endif; ?>
    </div>
    <div class="clear"></div>
    <div class="enquire col">
        ...
        <h3>Enquiry Form</h3>
        <?php $this->renderPartial('application.views.main._enquire', array('model' => $model, 'url' => Yii::app()->createAbsoluteUrl('/hotel/'.$model->id.'-'.Q::slug($model->name) )  )); ?>
    </div>
    <div class="clear"></div>
    ...
    <div class="row small">
        <?php
        $hotels = Q::getRelated($model->city, $model->city->country, $model->city->country->region, $model->id);
        if ($hotels) {
            ...
            foreach ($hotels as $data):
                $thumbnail = isset($data->featuredImage) ? $data->featuredImage->thumbnail_url : ( (isset($data->image))?$data->image->thumbnail_url:"");
                ?>
                <div class="col">
